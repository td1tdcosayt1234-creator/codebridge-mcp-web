...
export default function Tokens(){
  ...
  const [plan,setPlan]=useState("free"); useEffect(()=>{fetch("/api/auth/me").then(r=>r.json()).then(j=>setPlan(j?.user?.plan||"free")).catch(()=>{});},[]);
  const u=d?.usage||{mcpCalls:0,githubCalls:0,balance:0,usedTotal:0}; const limit=100;
  return (<div><h2>My Coins</h2>
  <div className="card card-glow" style={{marginBottom:12,display:"flex",gap:12,alignItems:"center",justifyContent:"space-between",flexWrap:"wrap"}}>
    <div><span className="muted small">Plan</span><div style={{fontSize:22,fontWeight:800,textTransform:"capitalize"}}>{plan} {plan==="free"?"🌱":plan==="pro"?"🚀":"🏢"}</div></div>
    {plan==="free"
      ? <a className="btn" href="/pricing">Upgrade →</a>
      : <span className="badge ok">Paid via Paddle</span>}
  </div>
