...
export default function Support(){
  const [s,setS]=useState(""); const [b,setB]=useState(""); const [hp,setHp]=useState(""); const [msg,setMsg]=useState(""); const [ok,setOk]=useState<boolean|null>(null); const [busy,setBusy]=useState(false);
  async function send(e:any){
    e.preventDefault();
    if(!s.trim()||!b.trim()){ setMsg("Please write both subject and message."); setOk(false); return; }
    setBusy(true);
  ...
  return (<div className="prose"><div className="page-hero" style={{textAlign:"center"}}><div className="kicker" style={{marginBottom:12}}><span className="pulse-dot" />Avg reply: fast</div><h1>Support — <span className="grad-anim">get help</span></h1><p style={{margin:"0 auto"}}>Open a ticket and the admin will reply. Check the quick fixes below first — most issues solve in a minute.</p></div>
  ...
  <div className="card card-glow" style={{maxWidth:600,margin:"18px auto 0"}}><h3 style={{marginTop:0}}>🎫 Open a ticket</h3><p className="muted small">Login is optional, but logged-in tickets get matched to you and answered faster. Max 5 tickets/hour.</p><form onSubmit={send}><label>Subject (e.g. build failed, MCP 404)</label><input value={s} onChange={e=>setS(e.target.value)} placeholder="Short title"/><label>Message (what you did, the error, run_id if any)</label><textarea rows={5} value={b} onChange={e=>setB(e.target.value)} placeholder="Write details..."/><input name="website" value={hp} onChange={e=>setHp(e.target.value)} autoComplete="off" tabIndex={-1} aria-hidden="true" style={{position:"absolute",left:"-9999px",opacity:0,height:0}}/><div style={{marginTop:12}}><button className="btn" style={{width:"100%"}} disabled={busy}>{busy?"Sending…":"Send ticket →"}</button></div></form>{msg&&<p className="small" style={{marginBottom:0}}><span className={"badge "+(ok?"ok":"bad")}>{msg}</span></p>}</div></div>);
}
