...
export async function GET(){
  ...
  return NextResponse.json({events:ev,usage:us,builds,key:key?.key||""});
}
export async function POST(req:Request){
  const t=cookies().get("session")?.value||""; const p=await verifyJwt(t); if(!p) return NextResponse.json({error:"auth"},{status:401});
  const {repo,branch}=await req.json();
  const db=await readDb();
  const b={id:uid("run"),userId:p.sub,repo:String(repo||"demo/repo"),branch:String(branch||"main"),status:"queued",log:"Triggered via dashboard. GitHub Actions workflow dispatch simulated. Push -> checkout -> build -> artifact.",at:new Date().toISOString()};
  db.builds.push(b);
