...
export type Task = { id:string; userId:string; title:string; prompt:string; kind:"compile"|"fix-compile"; files:{path:string; content:string}[]; status:"queued"|"running"|"done"|"failed"; log:string; result:string; runUrl:string; tokensEst:number; tokensCharged:number; createdAt:string; updatedAt:string; apkSize?:number };
