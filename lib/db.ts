...
export type TaskArtifact = { name:string; size:number };
export type Task = { id:string; userId:string; title:string; prompt:string; kind:"compile"|"fix-compile"; files:{path:string; content:string}[]; status:"queued"|"running"|"done"|"failed"; log:string; result:string; runUrl:string; tokensEst:number; tokensCharged:number; createdAt:string; updatedAt:string; apkSize?:number; jarSize?:number; jarName?:string; artifacts?:TaskArtifact[] };
...
// opaque â password hashes, tokens and logs cannot be read from disk.
...
function warnNoDbKey() {
  ...
  console.warn("[codebridge] SECURITY: DB_MASTER_KEY not set â database is stored in PLAINTEXT. Set it to enable at-rest encryption.");
}
...
function decryptDb(raw: string): string {
  ...
  if (t.startsWith('{"v":1') || t.startsWith('{"v": 1')) {
    ...
    if (!plain) throw new Error("Database decrypt failed â wrong DB_MASTER_KEY or corrupt file. NOT overwriting. Restore from data/db.json.bak.1 or fix the key.");
    return plain;
  }
  warnNoDbKey();
